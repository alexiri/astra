...
ansible-playbook -i ../envs/staging/ipa_inventory.ini freeipa_setup.yml -vvv
```
...
8. Downloads IPA CA certificate to `infra/ansible/ipa_ca.crt`

### Variables

Variables are passed via inventory file (generated by Terraform):

- `ipa_realm`: Kerberos realm (e.g., `ASTRA-STAGING.TEST`)
- `ipa_domain`: IPA domain (e.g., `astra-staging.test`)
...
- **IPA CA cert:** Downloaded to `infra/ansible/ipa_ca.crt`
...
- Check logs: `ssh ec2-user@<host> sudo cat /var/log/ipaserver-install.log`
